...
export class MountainScene {
	...
	drawMountainScene() {
		...
		a.reset(this.width * 0.04, this.height * 0.8);
		a.drawShape(TREE_CONE_LEFT, false, 0, 0, 0, TREE_FILL, TREE_FILL);
	...
	setAnimationSceneValues(percentage: number) {
		this.anCtx.clearRect(
			this.indicatorX - this.indicatorRadius,
			this.indicatorY - this.indicatorRadius,
			this.indicatorX,
			this.indicatorY
		);
		...
		const indicatorGradient = this.anCtx.createRadialGradient(
			this.indicatorX,
			this.indicatorY,
			this.indicatorRadius * 0.2,
			this.indicatorX,
			this.indicatorY,
			this.indicatorRadius * 0.9
		);
	...
	animate() {
		...
		if (this.counter % 2 === 0) {
			...
			const indicatorGradient = this.anCtx.createRadialGradient(
				...
				indicatorX,
				indicatorY,
				indicatorRadius * Math.sin(this.rad)
			);
			indicatorGradient.addColorStop(0, 'rgb(240, 150, 80)');
