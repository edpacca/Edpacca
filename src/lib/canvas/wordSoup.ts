...
export class WordSoup {
	...
	constructor(
		...
		Object.keys(words).forEach((word) => {
			this.wordBlocks.push(new WordBlock(word, getProportionalFontSize(fontArea, words[word], minFontSize), getColor(word)));
		});
	...
	setForces(value: Vector2D, randomForces: boolean = false, dampingFactor?: number) {
	...
	updateVelocity(word: WordBlock, collisionRects: Rect[], force: Vector2D) {
		...
		const potentialPos = new Vector2D(word.position.x + word.velocity.x, word.position.y + word.velocity.y);
