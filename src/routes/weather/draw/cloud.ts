...
export class Cloud {
  ...
  generate = () => {
    ...
    for (let i = 0; i < this.size; i++) {
      const orbRadius = randBetween(
        this.radius * ORB_SIZE_VARIANCE[0],
        this.radius * ORB_SIZE_VARIANCE[1],
      );
      const orbX = randBetween(
        ORB_X_SPREAD_FACTOR[0] * this.radius,
        ORB_X_SPREAD_FACTOR[1] * this.radius,
      );
      const orbY = randBetween(
        ORB_Y_SPREAD_FACTOR[0] * this.radius,
        ORB_Y_SPREAD_FACTOR[1] * this.radius,
      );
      if (orbX < smallestX) {
        smallestX = orbX;
      }
